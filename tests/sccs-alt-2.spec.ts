...
test.describe('Calculator: SCCS (alt-2)', () => {
  test('should calculate the correct sample size and power', async ({ page }) => {
    // 1. 페이지로 이동합니다.
    await page.goto('/calculator/sccs/alt-2');
    ...
    await page.getByTestId('rho').fill('3');
    await page.getByTestId('r').fill('0.115');
