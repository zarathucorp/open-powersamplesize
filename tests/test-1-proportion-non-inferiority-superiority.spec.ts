...
test.describe('Calculator: Test 1 Proportion (Non-Inferiority/Superiority)', () => {
  test('should calculate the correct sample size and power', async ({ page }) => {
    // 1. 페이지로 이동합니다.
    await page.goto('/calculator/test-1-proportion/non-inferiority-superiority');
    ...
    await page.getByTestId('p').fill('0.5');
    await page.getByTestId('p0').fill('0.3');
