...
test.describe('Calculator: Compare Paired Proportions (McNemar 1-Sided)', () => {
  test('should calculate the correct sample size and power', async ({ page }) => {
    // 1. 페이지로 이동합니다.
    await page.goto('/calculator/compare-paired-proportions/mcnemar-1-sided');
    ...
    await page.getByTestId('p01').fill('0.45');
    await page.getByTestId('p10').fill('0.05');
