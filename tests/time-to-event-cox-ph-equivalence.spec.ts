...
test.describe('Calculator: Time to Event (Cox PH Equivalence)', () => {
  test('should calculate the correct sample size and power', async ({ page }) => {
    // 1. 페이지로 이동합니다.
    await page.goto('/calculator/time-to-event/cox-ph-equivalence');
    ...
    await page.getByTestId('hr').fill('1');
    await page.getByTestId('pE').fill('0.8');
    await page.getByTestId('pA').fill('0.5');
    await page.getByTestId('delta').fill('0.5');
