...
test.describe('Calculator: Compare 2 Means (Non-Inferiority/Superiority)', () => {
  test('should calculate the correct sample size and power', async ({ page }) => {
    // 1. 페이지로 이동합니다.
    await page.goto('/calculator/compare-2-means/non-inferiority-superiority');
    ...
    await page.getByTestId('muA').fill('5');
    await page.getByTestId('muB').fill('5');
    await page.getByTestId('stdDev').fill('10');
    await page.getByTestId('delta').fill('5');
