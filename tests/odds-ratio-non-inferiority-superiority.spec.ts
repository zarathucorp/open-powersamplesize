...
test.describe('Calculator: Odds Ratio (Non-Inferiority/Superiority)', () => {
  test('should calculate the correct sample size and power', async ({ page }) => {
    // 1. 페이지로 이동합니다.
    await page.goto('/calculator/odds-ratio/non-inferiority-superiority');
    ...
    await page.getByTestId('pA').fill('0.4');
    await page.getByTestId('pB').fill('0.25');
    await page.getByTestId('delta').fill('0.2');
