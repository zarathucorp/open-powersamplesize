...
test.describe('Calculator: Test 1 Mean (Equivalence)', () => {
  test('should calculate the correct sample size and power', async ({ page }) => {
    // 1. 페이지로 이동합니다.
    await page.goto('/calculator/test-1-mean/equivalence');
    ...
    await page.getByTestId('nullHypothesisMean').fill('2');
    await page.getByTestId('delta').fill('0.05');
    await page.getByTestId('stdDev').fill('0.1');
