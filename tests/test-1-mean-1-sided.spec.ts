...
test.describe('Calculator: Test 1 Mean (1-Sided)', () => {
  test('should calculate the correct sample size and power', async ({ page }) => {
    // 1. 페이지로 이동합니다.
    await page.goto('/calculator/test-1-mean/1-sided');
    ...
    await page.getByTestId('mean').fill('115');
    await page.getByTestId('nullHypothesisMean').fill('120');
    await page.getByTestId('stdDev').fill('24');
