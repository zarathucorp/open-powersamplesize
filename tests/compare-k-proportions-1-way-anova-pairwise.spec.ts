...
test.describe('Calculator: Compare K Proportions (1-Way ANOVA Pairwise)', () => {
  test('should calculate the correct sample size and power', async ({ page }) => {
    // 1. 페이지로 이동합니다.
    await page.goto('/calculator/compare-k-proportions/1-way-anova-pairwise');
    ...
    await page.getByTestId('pA').fill('0.2');
    await page.getByTestId('pB').fill('0.4');
    await page.getByTestId('tau').fill('2');
