...
test.describe('Calculator: Time to Event (Cox PH 2-Sided)', () => {
  test('should calculate the correct sample size and power', async ({ page }) => {
    // 1. 페이지로 이동합니다.
    await page.goto('/calculator/time-to-event/cox-ph-2-sided');
    ...
    await page.getByTestId('hr').fill('2');
    await page.getByTestId('hr0').fill('1');
