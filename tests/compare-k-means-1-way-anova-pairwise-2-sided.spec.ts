...
test.describe('Calculator: Compare K Means (1-Way ANOVA Pairwise 2-Sided)', () => {
  test('should calculate the correct sample size and power', async ({ page }) => {
    // 1. 페이지로 이동합니다.
    await page.goto('/calculator/compare-k-means/1-way-anova-pairwise-2-sided');
    ...
    await page.getByTestId('muB').fill('10');
    await page.getByTestId('stdDev').fill('10');
    await page.getByTestId('tau').fill('1');
    ...
    await page.getByTestId('sampleSize').fill('63');
