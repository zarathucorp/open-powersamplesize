...
test.describe('Calculator: Compare 2 Proportions (Non-Inferiority/Superiority)', () => {
  test('should calculate the correct sample size and power', async ({ page }) => {
    // 1. 페이지로 이동합니다.
    await page.goto('/calculator/compare-2-proportions/non-inferiority-superiority');
    ...
    await page.getByTestId('pA').fill('0.85');
    await page.getByTestId('pB').fill('0.65');
    await page.getByTestId('kappa').fill('1');
    await page.getByTestId('delta').fill('-0.1');

    // 3-2. 'Calculate' 버튼을 클릭합니다.
    await page.getByRole('button', { name: 'Calculate' }).click();

    // 3-3. 계산된 표본 크기(B)가 올바른지 확인합니다.
    const sampleSizeInput = page.getByTestId('nB');
    ...
    await page.getByTestId('nB').fill('25');
