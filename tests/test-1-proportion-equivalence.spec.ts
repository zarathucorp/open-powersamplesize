...
test.describe('Calculator: Test 1 Proportion (Equivalence)', () => {
  test('should calculate the correct sample size and power', async ({ page }) => {
    // 1. 페이지로 이동합니다.
    await page.goto('/calculator/test-1-proportion/equivalence');
    ...
    await page.getByTestId('p').fill('0.6');
    await page.getByTestId('p0').fill('0.6');
